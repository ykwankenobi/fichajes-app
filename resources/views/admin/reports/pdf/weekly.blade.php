...
<head>
    <meta charset="UTF-8">
    <title>Informe semanal de fichajes</title>
...
<body>
    @php
        ...
        if ($isMonth) {
            $periodLabel = \Carbon\Carbon::createFromFormat('Y-m-d', $week . '-01')
                ->translatedFormat('F Y');
        }

        if (! $isMonth && preg_match('/^\d{4}-W\d{2}$/', (string) $week) === 1) {
            ...
            $periodLabel = 'Semana ' . (int) $weekNumber . ': ' .
                $weekStart->format('d/m/Y') . ' - ' . $weekEnd->format('d/m/Y');
        }
    @endphp

    <div class="header">
        <h1 class="title">Informe semanal de fichajes</h1>
    ...
    @if (($includeDailyDetails ?? false) && isset($dailyReport) && $dailyReport->isNotEmpty())
